...
画面下部の`Food Knowledge`では登録食品、Personal、Official、Fallback、Confidence、利用回数ランキング、最近追加、最近更新を確認できます。続く`Personal Food Master`ではactive foodとpending candidateを管理できます。JSON Import完了時にはFood Master、Official、Generic、Fallbackの解決件数を表示します。
